...
- **Resolution:**
    - Rewrote `GlobalExceptionHandlerMiddleware.InvokeAsync` to build the response with a status-keyed generic message (e.g. "An unexpected error occurred while processing the request. Please reference the correlationId when contacting support." for 500, "The request timed out…" for 504, with a safe fallback). The raw `Exception` object is no longer passed to `ProblemDetailsWithErrors`.
    - Switched the logger calls to the `LogError(exception, message, args)` / `LogWarning(exception, …)` overloads so the full stack trace and inner-exception chain are captured by the structured logger (App Insights / OpenTelemetry), giving operators the diagnosis they need without leaking it to clients.
    - Deleted the three `ProblemDetailsWithErrors` constructors that accepted `Exception` / `IEnumerable<Exception>`. The middleware was the only caller, so removing the constructors prevents future code from accidentally re-introducing the leak. A code comment in `ProblemDetailsWithErrors.cs` documents why.
    - Kept the existing correlation-ID surface (`Extensions["correlationId"]`, `Extensions["traceId"]`) so support flows still work — the correlation ID is now the only handle exposed to clients.
    - Added `DerpCode.API.Tests/Middleware/GlobalExceptionHandlerMiddlewareTests.cs` (8 tests) covering: 500/504 status mapping, generic-message body, exception messages and dependency identifiers (`Npgsql`, `PostgresException`, inner messages) being absent from the response, the full exception object being threaded into `LogError`, correlation ID round-tripping from the caller, and the `Response.HasStarted` short-circuit.
